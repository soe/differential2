DiffView = {
	create: function (params) {
		...
		// to keep as global value, so as to save querying from DOM everytime
		var inner_h = 0; // the height of the viewing "window"
		var total_h = 0; // the height of all of the contents
		var indicator_h = 0; // the height of the scroll indicator

		var indicator = $("<div id='diff_overview_indicator'/>");
		overview.append(indicator);

		// Set up handler for scrolling the mainview pane
		mainview.scroll(function(e) {
			if(!inner_h) inner_h = inner.height();
			if(!total_h) total_h = table.height();
			if(!indicator_h) indicator_h = indicator.height();

			var pos = mainview.scrollTop();

			var y = pos / (total_h - inner_h) * inner_h;

			// Set position of the indicator
			if(y > inner_h - indicator_h) y = inner_h - indicator_h;
			indicator.css("top", y +"px");
		});
		...
		function doScroll(e) {
			...
			if(leftButtonDown) {
				...
				var pos = (total_h - inner_h) * (y / inner_h);
				pos = Math.min(total_h, Math.max(0, pos));

				// Set scroll position of mainview
